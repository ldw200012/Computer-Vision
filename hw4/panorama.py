...
def harris_corners(img, window_size=3, k=0.04):
    ...
    for row in range(H):
        for col in range(W):
            Ixy=np.sum(convolve(window, dxy[row : row+ws, col : col+ws]))
            Ix2=np.sum(convolve(window, dx2[row : row+ws, col : col+ws]))
            Iy2=np.sum(convolve(window, dy2[row : row+ws, col : col+ws]))
            M=[[Ix2,Ixy],[Ixy,Iy2]]
            response[row,col] = np.linalg.det(M) - (k * np.square(np.trace(M)))
...
def simple_descriptor(patch):
    ...
    sigma = np.std(patch) if np.std(patch) != 0 else 1
    mean = np.mean(patch)
    patch = (patch-mean)/sigma
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(N):
        temp_dists = np.sort(dists[i])
        if abs(temp_dists[0] / temp_dists[1]) < threshold:
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for i in range(rows):
        for j in range(cols):
            G = G_cells[i,j]
            theta = theta_cells[i,j]
            for k in range(G.shape[0]):
                for l in range(G.shape[1]):
                    low_bin = (theta[k,l] // degrees_per_bin) % n_bins
                    cells[i,j,int(low_bin)] += G[k,l]
